...
function checkIfProjectOpen(callback: Function) {
    ...
    if (vscode.workspace.workspaceFolders === undefined) {
        vscode.window.showErrorMessage('Open or create a project folder first');
    } else {
    ...
    vscode.window.showOpenDialog({
        ...
        if (e !== undefined) {
            vscode.window.showInputBox({
                ...
                console.log(f);
                if (f !== undefined && f !== "") {
                    fs.mkdirSync(e[0].path + "/" + f);
                    let fd = fs.openSync(e[0].path + "/" + f + "/" + f + ".pde", 'w');
                    fs.closeSync(fd);
                    
                    vscode.commands.executeCommand('vscode.openFolder', vscode.Uri.file(e[0].path + "/" + f));
                }
            });
        }
    });
...
function openExistingFolder() {
    vscode.window.showOpenDialog({
        ...
        if (e !== undefined) {
            vscode.commands.executeCommand('vscode.openFolder', vscode.Uri.file(e[0].path));
        }
    });
}
...
export function activate(context: vscode.ExtensionContext) {
    ...
    if (vscode.workspace.workspaceFolders !== undefined) {
        // workspaceFolders is guaranteed to be defined since we checked it above
        let root = vscode.workspace.workspaceFolders[0];
        let path = vscode.Uri.file(root.uri.fsPath + '/' + root.name + '.pde');
        vscode.window.showTextDocument(path);
    }

    console.log(vscode.workspace.workspaceFolders);
    
    let runItem: vscode.StatusBarItem;
    
    runItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    runItem.command = 'processing.RunTaskFile';
    runItem.text = "$(play) Run Project";
    
    context.subscriptions.push(runItem);
    runItem.show();
    
    let createItem: vscode.StatusBarItem;
    
    vscode.commands.registerCommand('processing.CreateWorkspace', openNewFolder);
    createItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    createItem.text = "$(add) New Project";
    createItem.command = "processing.CreateWorkspace";
    createItem.show();
    
    let openItem: vscode.StatusBarItem;
    
    vscode.commands.registerCommand('processing.OpenWorkspace', openExistingFolder);
    openItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    openItem.text = "$(folder-opened) Open Project";
    openItem.command = 'processing.OpenWorkspace';
    openItem.show();
